...
class FileInput extends \kartik\base\InputWidget
{
     ...
     */
    public $messageOptions = ['class' => 'alert alert-ie alert-danger'];
    ...
    public function init()
    {
        ...
        if ($this->showMessage) {
            ...
            $message = '<i class="fa fa-remove remove-flash"></i><strong>' . Yii::t('fileinput', 'Note:') . '</strong> ' . Yii::t('fileinput', 'Your browser does not support {validation}. Try an alternative or more recent browser to access these features.', ['validation' => $validation]);
          //  $content = Html::tag('div', $message, $this->messageOptions) . "<script>{$script};</script>";
            $content = Html::tag('div', $message, $this->messageOptions);
             $script = "<script>$('#". $this->options['id'] ."').parent().prepend('$content');</script>";
            $content = "{$script}";
